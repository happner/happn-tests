...
for (var testFileIndex in testFilenames) {
    ...
    if (testFilename.indexOf('09-datasource') != 0 && testFilename.indexOf('only.09-datasource') != 0) continue; //we only use files that start with 'test-'
    ...
    describe(testFilename.replace('.js', ''), function() {
        ...
        var getClient = function(service, callback){
            testInstance.client(service, function(e, instance) {
        ...
        before('should initialize the services', function(callback) {
            ...
            var serviceConfigs = [
                testInstance.serviceConfig1(tempFile, test_id),
                testInstance.serviceConfig2(tempFile1, test_id)
            ];
